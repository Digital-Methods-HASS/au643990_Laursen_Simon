...
#renaming columne with base R, as the dplyr::rename() method doesn't make permanent changes. 
#i am also changing the name entity to country. 
```

2) The facet is great, but geom_point(scatterplot) is not the best choice for showing long-term data. Can you create other type of visualisation for a 
long-term trend? 

## Task 2 -- Visualisation for long-tern trend

```{r}

LongTerm  <- ggplot(Western_Europe, aes(x=Year, y=Homicides, color = Country)) +
  geom_line() 
LongTerm

#Changed from geom_point to geom_line to create a better choice for showing long term data. Saving the ggplot in a vector, so it is ready for reuse

```

3) Can you change the facet layout to two columns and three rows so that the trends are more visible? 

## Task 3 -- Changing facet layout
```{r}
LongTermTrends <- LongTerm +
...
#Adding facet wrap with to the previous vector LongTerm. By adding the facewrap i split on the value Country and display them in three rows (nrows)
#Once again i save it in the vector LongTermTrends for easier use in the future. 

```

4) Move the legend from the current position on the side to below the facets, and label it "Country" instead of "Entity".

## Task 4 -- Moving the position of the legend 

```{r - moving the legend}
 
HomicideRates <- LongTermTrends + theme(legend.position="bottom") 

HomicideRates

#Changeing the position of the legend with the theme() function. The renaming of Entity to country, was changed in task 2. 
```
...
## Task 5 -- Adding floating table of contens and generating automatical timestamps

To add a floating table of content and automatically generate a timestamp to show when the document was updated, the following is added to the YAML: 
...
"rys.Date()" adds the automatical timestamp
"toc:true" adds the table of content
"toc_float:true" makes the table of content float
theme makes it red. 
